         ...
         <div className="max-w-7xl mx-auto px-6 relative z-10">
            <div className="text-center mb-16 max-w-5xl mx-auto">
               <motion.div initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} viewport={{once: true}} className="space-y-6">
                  <h2 className="text-4xl md:text-5xl font-black text-[#0a4d4a] uppercase tracking-tighter">Preventive Health Check-up Packages</h2>
               ...
               >
                  ...
                  <p className="text-gray-700 text-base md:text-lg leading-relaxed font-medium italic">
                     "Our carefully curated wellness packages cater to different health needs and lifestyles, helping clients take proactive control of their health. These
                     check-ups are aimed at promoting overall wellbeing, preventing illness, and enabling early detection and timely treatment - paving the way to a healthier,
                     longer life and true WELLNESS."
                  </p>
               </motion.div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto items-stretch mt-16">
               {packages.map((pkg, index) => (
                  <motion.div
                     ...
                     viewport={{once: true}}
                     className={`relative p-8 md:p-12 rounded-[3rem] border transition-all duration-500 flex flex-col h-full bg-white shadow-2xl hover:shadow-teal-900/10 ${
                        pkg.featured ? "border-teal-500 ring-4 ring-teal-500/5" : "border-teal-100"
                     }`}
                  >
                     {pkg.featured && (
                     ...
                     <h3 className="text-2xl md:text-3xl font-black text-[#0a4d4a] mb-10 uppercase tracking-tighter border-b border-teal-50 pb-6">{pkg.name}</h3>

                     {pkg.name === "Premium Health Screening" ? (
                        <p className="text-center font-bold text-xl text-[#0a4d4a]">
                           Standard Health Screening <br /> <strong className="text-3xl ">+</strong>
                        </p>
                     ) : (
                        ""
                     )}
